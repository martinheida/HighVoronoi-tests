...
const index_parallel_unbounded_nongeneral = 8
    ...
    for i in 1:length(nodes_)
        ...
        for kk in 1:repeat[i]
            try
                ...
                GC.gc()
                time_matrix[index_parallel_unbounded,i] += t2 = @elapsed VoronoiGeometry(xs, search_settings=_ss3 = (method=RCOriginal,threading=th_mode), integrate=false, silence=silence)
                printall && println("unbounded: nodes=$number, repeat=$kk, time=$t2")
                GC.gc()
                time_matrix[index_parallel_unbounded_nongeneral,i] += t3 = @elapsed VoronoiGeometry(xs, search_settings=_ss3 = (method=RCNonGeneral,threading=th_mode), integrate=false, silence=silence)
                printall && println("unbounded_nongeneral: nodes=$number, repeat=$kk, time=$t3")
                GC.gc()
                time_matrix[index_parallel_bounded,i] += t4 = @elapsed VoronoiGeometry(xs,cube, search_settings=_ss3 = (method=RCOriginal,threading=th_mode), integrate=false, silence=silence)
                printall && println("bounded: nodes=$number, repeat=$kk, time=$t4")
                GC.gc()
                time_matrix[index_parallel_bounded_nongeneral,i] += t5 = @elapsed VoronoiGeometry(xs,cube, search_settings=_ss3 = (method=RCNonGeneral,threading=th_mode), integrate=false, silence=silence)
                printall && println("bounded_nongeneral: nodes=$number, repeat=$kk, time=$t5")
                GC.gc()
                time_matrix[index_parallel_bounded_combined,i] += t6 = @elapsed VoronoiGeometry(xs,cube, search_settings=_ss3 = (method=RCCombined,threading=th_mode), integrate=false, silence=silence)
                ...
                time_matrix[index_parallel_unbounded,i] -= t2 # @elapsed VoronoiGeometry(xs, search_settings=_ss3 = (method=RCOriginal,threading=th_mode), integrate=false, silence=true)
                time_matrix[index_parallel_unbounded_nongeneral,i] -= t3 # @elapsed VoronoiGeometry(xs, search_settings=_ss3 = (method=RCNonGeneral,threading=th_mode), integrate=false, silence=true)
                time_matrix[index_parallel_bounded,i] -= t4 # @elapsed VoronoiGeometry(xs,cube, search_settings=_ss3 = (method=RCOriginal,threading=th_mode), integrate=false, silence=true)
                time_matrix[index_parallel_bounded_nongeneral,i] -= t5 # @elapsed VoronoiGeometry(xs,cube, search_settings=_ss3 = (method=RCNonGeneral,threading=th_mode), integrate=false, silence=true)
                time_matrix[index_parallel_bounded_combined,i] -= t6 # @elapsed VoronoiGeometry(xs,cube, search_settings=_ss3 = (method=RCCombined,threading=th_mode), integrate=false, silence=true)
                t1 = t2 = t3 = t4 = t5 = t6 = 0.0
            end
        end
        count != 0 && (time_matrix[:,i] ./= count) 
    end
    file_name = "results-multi-para-$(dim)D.txt"
...
#error()
repeat_array = [6  , 6  , 6   , 6   , 5   , 3    , 3    , 3    , 3    , 3    , 3    , 3    , 3    ]
run_tests(nodes_array[1:13], 5, repeat_array, true,false)
